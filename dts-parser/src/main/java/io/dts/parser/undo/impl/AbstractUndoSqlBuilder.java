...
public abstract class AbstractUndoSqlBuilder implements ITxcUndoSqlBuilder {

	private final RollbackInfor txcUndoLogRollbackInfor;

	public AbstractUndoSqlBuilder(RollbackInfor txcUndoLogRollbackInfor) {
	...
	protected String fieldsValueSerialization(List<TxcField> fields) {
		...
		for (int i = 0; i < fields.size(); i++) {
			...
			appender.append(TxcObjectWapper.jsonObjectDeserialize(field.getFieldType(), field.getFieldValue()));
		}

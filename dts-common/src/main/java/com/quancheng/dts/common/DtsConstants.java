...
public class DtsConstants {
	...
	public static boolean isVersion_1_0(Channel c) {
		return DtsConstants.VERSION_1_0.equalsIgnoreCase(versionMap.get(c.remoteAddress()
				.toString()));
	}
	...
	public static boolean isLargeThanVersion_1_0(Channel c) {
		if (SKIP_VER_CHK)
			return true;
		return (versionMap.get(c.remoteAddress().toString()).compareTo(
				DtsConstants.VERSION_1_0) > 0) ? true : false;
	}

	public static boolean isVersion_1_0_3(Channel c) {
		return DtsConstants.VERSION_1_0_3.equalsIgnoreCase(versionMap.get(c.remoteAddress()
				.toString()));
	}

	public static void setVersion(ChannelHandlerContext ctx, String version) {
		ctx.attr(DtsConstants.VERSIONKEY).set(version);
	}

	public static boolean isVersion_1_0(ChannelHandlerContext ctx) {
		return DtsConstants.VERSION_1_0.equalsIgnoreCase(ctx.attr(VERSIONKEY).get());
	}

	public static boolean isVersion_1_0_3(ChannelHandlerContext ctx) {
		return DtsConstants.VERSION_1_0_3.equalsIgnoreCase(ctx.attr(VERSIONKEY).get());
	}

	public static boolean isVersion_1_0(String version) {
		return DtsConstants.VERSION_1_0.equalsIgnoreCase(version);
	}

	public static boolean isVersion_1_0_3(String version) {
		return DtsConstants.VERSION_1_0_3.equalsIgnoreCase(version);
	}
}
